...
public class HistoryList {
    ...
    public void undo() {
        if (commandlist.size() > 0) {
            Command commandObject = (Command) ((ArrayList<Command>) commandlist).get(commandlist.size() - 1);((ArrayList<Command>) commandlist).remove(commandObject);
    ...
    public void redo() {
        if (undolist.size() > 0) {
            Command commandObject = (Command) ((ArrayList<Command>) undolist).get(undolist.size() - 1);commandObject.execute();
